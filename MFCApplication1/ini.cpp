...
void CIni::AddModulPath(CString& rstrFileName, bool bModulPath)
{
    ...
    _tsplitpath_s(rstrFileName, drive, dir, fname, ext);
    if (!drive[0])
    {
        ...
        _tsplitpath_s(strModule, drive, dir, fname, ext);
        strModule = drive;
        strModule += dir;
...
CString CIni::GetDefaultIniFile(bool bModulPath)
{
    ...
    _tsplitpath_s(strTemp, drive, dir, fname, ext);
    strTemp = fname;
    strTemp += _T(".ini");
    if (bModulPath)
    {
...
double CIni::GetDouble(LPCTSTR lpszEntry, double fDefault, LPCTSTR lpszSection)
{
    ...
    _sntprintf_s(szDefault, _countof(szDefault), _T("%g"), fDefault);
...
{
    TCHAR szDefault[MAX_PATH];
    _sntprintf_s(szDefault, _countof(szDefault), _T("%d"), nDefault);
    szDefault[_countof(szDefault) - 1] = _T('\0');
    GetLPCSTR(lpszEntry, lpszSection, szDefault);
...
{
    TCHAR szDefault[MAX_PATH];
    _sntprintf_s(szDefault, _countof(szDefault), _T("%I64u"), nDefault);
    szDefault[_countof(szDefault) - 1] = _T('\0');
    GetLPCSTR(lpszEntry, lpszSection, szDefault);
    ULONGLONG nResult;
...
{
    TCHAR szDefault[MAX_PATH];
    _sntprintf_s(szDefault, _countof(szDefault), _T("%u"), nDefault);
    szDefault[_countof(szDefault) - 1] = _T('\0');
    GetLPCSTR(lpszEntry, lpszSection, szDefault);
...
{
    TCHAR szDefault[MAX_PATH];
    _sntprintf_s(szDefault, _countof(szDefault), _T("%d"), bDefault);
    szDefault[_countof(szDefault) - 1] = _T('\0');
    GetLPCSTR(lpszEntry, lpszSection, szDefault);
...
CRect CIni::GetRect(LPCTSTR lpszEntry, CRect rectDefault, LPCTSTR lpszSection)
{
    ...
    //new Version found
    if (_stscanf_s(strRect, _T("%d,%d,%d,%d"), &rectDefault.left, &rectDefault.top, &rectDefault.right, &rectDefault.bottom) == 4)
        return rectReturn;
    //old Version found
    if (_stscanf_s(strRect, _T("(%d,%d,%d,%d)"), &rectReturn.top, &rectReturn.left, &rectReturn.bottom, &rectReturn.right) != 4)
        return rectDefault;
    return rectReturn;
}
...
void CIni::WriteDouble(LPCTSTR lpszEntry, double f, LPCTSTR lpszSection)
{
    ...
    _sntprintf_s(szBuffer, _countof(szBuffer), _T("%g"), f);
...
void CIni::WriteBool(LPCTSTR lpszEntry, bool b, LPCTSTR lpszSection)
{
    if (lpszSection != NULL)
        m_strSection = lpszSection;
    TCHAR szBuffer[MAX_PATH];
    _sntprintf_s(szBuffer, _countof(szBuffer), _T("%d"), (int)b);
    szBuffer[_countof(szBuffer) - 1] = _T('\0');
    WritePrivateProfileString(m_strSection, lpszEntry, szBuffer, m_strFileName);
}
