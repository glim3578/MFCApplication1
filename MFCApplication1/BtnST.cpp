...
{
	if (m_bParamSet)
		::SendMessage(t_hWnd, m_dw_WM_RESERVED_ID, 1, 0);
	m_bState = TRUE;
...
{
	if (m_bParamSet)
		::SendMessage(t_hWnd, m_dw_WM_RESERVED_ID, 0, 0);
	m_bState = FALSE;
	CButton::OnLButtonUp(nFlags, point);
}

void CButtonST::SetParameter(HWND target_handle, DWORD WM_ID)
{
	m_bState = FALSE;
	m_bParamSet = TRUE;
	t_hWnd = target_handle;
	m_dw_WM_RESERVED_ID = WM_ID;
}
...
DWORD CButtonST::SetURL(LPCTSTR lpszURL)
{
	...
	if (lpszURL)
	{
		...
		wcsncpy_s(m_szURL, lpszURL, _MAX_PATH);
	} // if
...
DWORD CButtonST::SetSound(LPCTSTR lpszSound, HMODULE hMod, BOOL bPlayOnClick, BOOL bPlayAsync)
{
	...
	if (lpszSound)
	{
		...
		else
		{
			...
			wcscpy_s(m_csSounds[byIndex].szSound, lpszSound);
